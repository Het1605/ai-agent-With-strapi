...
async def schema_execution_planner_agent(state: AgentState) -> AgentState:
    ...
    system_prompt = (
        ...
        "3. DUPLICATE PREVENTION: You MUST NOT add any table to the output that already exists in the 'EXISTING SCHEMA' list above. Use the 'slug' or 'table_name' for comparison.\n"
        "4. Output Filtering: Only output the tables that were provided in the input JSON, but sorted correctly.\n"
        "5. Metadata Preservation (CRITICAL): You MUST preserve ALL fields within each table object, especially 'slug', 'singular_name', 'plural_name', and 'display_name'. The 'slug' is the PRIMARY STRAPI IDENTIFIER.\n\n"
