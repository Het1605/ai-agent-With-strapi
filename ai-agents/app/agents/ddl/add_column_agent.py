...
async def add_column_agent(state: AgentState) -> AgentState:
    ...
    system_prompt = (
        ...
        "2. DYNAMIC CONSTRAINTS: Only include attributes (required, unique, default, minLength, etc.) if the user explicitly mentions them or if they are logically essential for the type in the 'field_registry'.\n"
        "3. CLEAN OUTPUT: Do NOT include 'false' or 'null' constraints. Only include meaningful attributes found in the 'field_registry' for that type.\n"
        "4. Use schema_data as the source of truth — merge new columns, do not rebuild.\n"
