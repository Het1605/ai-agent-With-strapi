...
async def create_table_agent(state: AgentState) -> AgentState:
    ...
    system_prompt = (
        ...
        "7. DEPENDENCY ORDERING: When generating multiple tables, you MUST output them in creation order. Parent tables (those being referenced) MUST appear in the 'tables' array BEFORE dependent tables (those containing the relation field).\n"
        "   Example: If 'employee' depends on 'company', 'company' MUST be the first item in the array.\n\n"
        "DATABASE CONTEXT:\n"
